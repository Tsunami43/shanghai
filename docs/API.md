...
> **Status:** A run merges each selected group into a single segment. Every
> entry is preserved, so only the per-segment file headers are reclaimed;
> discarding entries covered by a snapshot is not implemented.
