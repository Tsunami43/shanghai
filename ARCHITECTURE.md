...
| Replication | distributed | `Leader`/`Follower`/`Stream`/`Monitor` with quorum acks; entry delivery, offset acks and catch-up all run over Erlang distribution; per-group role assignment and leader failover via `GroupCoordinator`; promotion is gated on a majority vote and entries are fenced by leadership epoch, so a partitioned minority cannot become writable; leader and follower persist to the WAL when it is running; not Raft (no log matching/commit index); votes are persisted before being granted, so a restart cannot undo one; a group configured without `:members` runs unfenced |
| Observability | implemented | telemetry + `MetricsReporter`; Admin HTTP API and Prometheus `/metrics` live in `admin_api` |
| Admin/Ops | partial | `admin_api` HTTP surface (`/health`, `/ready`, `/metrics`, `/api/v1/{info,status,nodes,nodes/:id,replicas,snapshots,kv,kv/:key,metrics}`, per-request logging), with `/ready` delegating to `Admin.Health` (the shared subsystem-health aggregate in the `admin` library app); `shanghaictl` is a standalone escript client (`mix escript.build` in `apps/shanghaictl`) talking to the Admin API over HTTP; config hot-reload, dashboards, deploy are on the roadmap |
