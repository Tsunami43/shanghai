defmodule Cluster.State do
  ...
  @doc """
  Returns the ids of the nodes in the given id namespace, sorted by their string
  value.
  """
  @spec node_ids_in_namespace(t(), String.t()) :: [NodeId.t()]
  def node_ids_in_namespace(%__MODULE__{} = cluster, namespace) when is_binary(namespace) do
    cluster
    |> nodes_in_namespace(namespace)
    |> Enum.map(& &1.id)
  end
  ...
  def up_by_namespace(%__MODULE__{} = cluster) do
    cluster
  ...
  @doc """
  Returns the host of the node with `node_id`, or `nil` when the node is not a
  member.
  """
  @spec host_of(t(), NodeId.t()) :: String.t() | nil
  def host_of(%__MODULE__{nodes: nodes}, node_id) do
    case Map.fetch(nodes, node_id) do
      {:ok, node} -> node.host
      :error -> nil
    end
