defmodule Cluster.StateTest do
  ...
  describe "count_in_namespace/2" do
    test "counts nodes in an id namespace" do
      cluster = State.new(NodeId.new("local"))
      ...
      assert State.count_in_namespace(cluster, "eu") == 2
      assert State.count_in_namespace(cluster, "ap") == 0
    end
  end

  describe "node_ids_in_namespace/2" do
    test "returns sorted node ids in a namespace" do
      cluster = State.new(NodeId.new("local"))
      {:ok, cluster} = State.add_node(cluster, Node.new(NodeId.new("eu-2"), "h", 4002))
      {:ok, cluster} = State.add_node(cluster, Node.new(NodeId.new("eu-1"), "h", 4001))
      {:ok, cluster} = State.add_node(cluster, Node.new(NodeId.new("us-1"), "h", 4003))

      assert Enum.map(State.node_ids_in_namespace(cluster, "eu"), & &1.value) == ["eu-1", "eu-2"]
      assert State.node_ids_in_namespace(cluster, "ap") == []
    end
  end
  ...
  describe "host_count/1" do
    test "counts distinct hosts" do
      ...
      assert State.host_count(cluster) == 2
      assert State.host_count(State.new(NodeId.new("solo"))) == 0
    end
  end
  ...
  describe "namespace_counts/1" do
    test "counts nodes per id namespace" do
      cluster = State.new(NodeId.new("local"))
      {:ok, cluster} = State.add_node(cluster, Node.new(NodeId.new("eu-1"), "h", 4001))
      {:ok, cluster} = State.add_node(cluster, Node.new(NodeId.new("eu-2"), "h", 4002))
      {:ok, cluster} = State.add_node(cluster, Node.new(NodeId.new("us-1"), "h", 4003))

      assert State.namespace_counts(cluster) == %{"eu" => 2, "us" => 1}
    end
  end

  describe "unavailable_node_ids/1" do
    test "returns ids of down and suspect nodes, sorted" do
      cluster = State.new(NodeId.new("local"))
      {:ok, cluster} = State.add_node(cluster, Node.new(NodeId.new("n1"), "h", 4001))
      {:ok, cluster} = State.add_node(cluster, Node.new(NodeId.new("n2"), "h", 4002))
      {:ok, cluster} = State.mark_node_down(cluster, NodeId.new("n2"))

      assert Enum.map(State.unavailable_node_ids(cluster), & &1.value) == ["n2"]
    end
  end
  ...
  describe "up_by_host/1" do
    test "counts up nodes per host" do
      cluster = State.new(NodeId.new("local"))
      {:ok, cluster} = State.add_node(cluster, Node.new(NodeId.new("n1"), "hostA", 4001))
      {:ok, cluster} = State.add_node(cluster, Node.new(NodeId.new("n2"), "hostA", 4002))
      {:ok, cluster} = State.add_node(cluster, Node.new(NodeId.new("n3"), "hostB", 4003))
      {:ok, cluster} = State.mark_node_down(cluster, NodeId.new("n3"))

      assert State.up_by_host(cluster) == %{"hostA" => 2}
    end
  end
